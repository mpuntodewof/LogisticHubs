...
- **Moka POS, iReap** — < IDR 500K/month — 1-5 employees — Single store — POS only
- **Majoo, Pawoon** — IDR 500K-2M/month — 5-20 employees — 1-3 locations — POS + basic inventory
- **NiagaOne ERP targets this space** — 20-1000 employees — 1-10+ locations — Full operations + e-commerce
- **SAP Business One, HashMicro, Oracle NetSuite** — IDR 10M+/month — 1000+ employees — Enterprise — Full ERP suite
...
**1. LOGISTICS DNA**
Built FROM a logistics system. Shipping, tracking, warehouse management are core — not bolted on. No competitor at this price point has this.

**2. TRUE OMNICHANNEL AS DEFAULT**
Not "POS + e-commerce add-on." The system IS omnichannel. Inventory is one pool, allocated across channels.

**3. MID-MARKET FIT**
Complex enough for 200+ employee operations. Simple enough that a kasir can learn in 30 minutes. Priced for IDR 2-5M/month, not IDR 20M.

**4. INDONESIA-NATIVE**
PPN 12% built into every transaction. e-Faktur export. QRIS/GoPay/OVO/VA as first-class payment methods. Bahasa Indonesia is the PRIMARY language, not a translation. IDR currency with proper thousand-separator (titik, not koma).

**5. MODERN TECH, SOLO-DEV EFFICIENT**
.NET 8 + Blazor = one language (C#) for API + frontend. Clean Architecture = maintainable as it scales. Multi-tenant from Day 1 = operational efficiency.
...
**YEAR 1 — LOCAL MARKET KNOWLEDGE**
- PPN baked in
- QRIS/eWallet
- Bahasa first
- IDR native
- Local biz practices
- (Easy to start but hard to replicate correctly)

**YEAR 2 — SWITCHING COSTS**
- Years of transaction data
- Staff trained
- Integrations configured
- (Grows with each month of usage)

**YEAR 3+ — DATA NETWORK EFFECTS**
- Industry benchmarks
- Demand forecasting
- Pricing optimization
- Supplier network
- (Unique asset from aggregate tenant data)
...
- **Application Tier** — Shared .NET 8 API + Blazor Frontend
    - Tenant Resolution Middleware — subdomain: {tenant}.niagaone.id OR header: X-Tenant-Id → resolves to tenant connection string
    - Tenant-Aware DbContext — Connection string swapped per request
- **Tenant Databases** — Each tenant gets their own MySQL database (tenant_001, tenant_002, tenant_003) containing Products, Orders, Inventory, etc.
- **Master Database** — master_db containing Tenants, Plans, Billing, Users (cross-tenant auth)
